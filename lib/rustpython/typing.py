...
def _should_unflatten_callable_args(typ, args):
    ...
    """
    return (
        typ.__origin__ is col_abc.Callable
...
class _GenericAlias(_BaseGenericAlias, _root=True):
    ...
    def _make_substitution(self, args, new_arg_by_param):
        ...
        for old_arg in args:
            ...
            if self.__origin__ == col_abc.Callable and isinstance(new_arg, tuple):
...
def _proto_hook(cls, other):
    ...
    for attr in cls.__protocol_attrs__:
        for base in other.__mro__:
            ...
            annotations = getattr(base, '__annotations__', {})
            if (isinstance(annotations, col_abc.Mapping) and
...
Hashable = _alias(col_abc.Hashable, 0)  # Not generic.
Awaitable = _alias(col_abc.Awaitable, 1)
Coroutine = _alias(col_abc.Coroutine, 3)
AsyncIterable = _alias(col_abc.AsyncIterable, 1)
AsyncIterator = _alias(col_abc.AsyncIterator, 1)
Iterable = _alias(col_abc.Iterable, 1)
Iterator = _alias(col_abc.Iterator, 1)
Reversible = _alias(col_abc.Reversible, 1)
Sized = _alias(col_abc.Sized, 0)  # Not generic.
Container = _alias(col_abc.Container, 1)
Collection = _alias(col_abc.Collection, 1)
Callable = _CallableType(col_abc.Callable, 2)
Callable.__doc__ = \
    ...
    """
AbstractSet = _alias(col_abc.Set, 1, name='AbstractSet')
MutableSet = _alias(col_abc.MutableSet, 1)
# NOTE: Mapping is only covariant in the value type.
Mapping = _alias(col_abc.Mapping, 2)
MutableMapping = _alias(col_abc.MutableMapping, 2)
Sequence = _alias(col_abc.Sequence, 1)
MutableSequence = _alias(col_abc.MutableSequence, 1)
ByteString = _DeprecatedGenericAlias(
    col_abc.ByteString, 0, removal_version=(3, 14)  # Not generic.
)
...
MappingView = _alias(col_abc.MappingView, 1)
KeysView = _alias(col_abc.KeysView, 1)
ItemsView = _alias(col_abc.ItemsView, 2)
ValuesView = _alias(col_abc.ValuesView, 1)
Dict = _alias(dict, 2, inst=False, name='Dict')
DefaultDict = _alias(collections.defaultdict, 2, name='DefaultDict')
OrderedDict = _alias(collections.OrderedDict, 2)
Counter = _alias(collections.Counter, 1)
ChainMap = _alias(collections.ChainMap, 2)
Generator = _alias(col_abc.Generator, 3, defaults=(types.NoneType, types.NoneType))
AsyncGenerator = _alias(col_abc.AsyncGenerator, 2, defaults=(types.NoneType,))
